...
import pdfkit
from checkanswers import check
...
def login():
    ...
    return render_template('loginpagefirebase.html')
...
def dashboard():
    ...
    """
    # If the user comes directly to the dashboard without the login page, it checks the session to check if the user
    # is signed in
    if 'username' in session.keys():
...
def quiz():
    ...
    """
    # Gets url argument specifying if the user wants a quiz in progress or wants to start a new one.
    if request.method != 'POST':
        get_quiz_in_progress = request.args.get(
            'quiz_in_progress')
        # If there's a quiz in progress
        if get_quiz_in_progress == 'true':
            ...
            if quiz_in_progress is not None:
                ...
                return render_template('quizinprogress.html', quizqa=quiz_in_progress['results'],
...
def logout():
    ...
    # Sends them back to login screen
    return redirect(url_for('login'))
...
def save_and_get_quiz_results():
    ...
    if request.method == 'POST':
        ...
        if questions_answers is not None:
            ...
            # saves results and returns the datetime
            date_time = quiz_ref.save_results(session['username'], results,
                                              score, timetaken)
            # Deletes the quiz in progress
            quiz_ref.delete_quiz_in_progress(session['username'])
            # Sends them to the report generation page with the date and time the quiz was submitted as the url argument
